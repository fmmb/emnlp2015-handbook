...
# FMMB: The first replacement avoids problems with files such as .DS_Store.
# FMMB: THe NAACL version changed the line again. keeping that one
# paper_ids = [int(n) for n in os.listdir(fdir)]
# paper_ids = [int(n) for n in os.listdir(fdir) if os.path.isdir(fdir+"/"+n)]
