...
while (my $line = <STDIN>) {
  ...
  # Added later (FMMB): for posters related with other oral presentations
  # e.g: ! Crosslingual Annotation ... %by Frances Yung ...
  elsif ($line =~ /^\!\s+(.*?)($tspanpat)?\s*(.*)/i) { 
    my $pre   = $1;
    my $time  = $2;
    $time = "\$\\bullet\$" unless $time;
    ...
    $time =~ s/(\d+):(\d+)/timef($1,$2)/eg;
    my $author = (exists $hash->{'%by'} ? "$hash->{'%by'}" : "");
    #print STDERR "DEBUG: -> $pre\n-> $post\n-> $start\n-> $end\n$author";
    print "\\item[$time] {$pre$post}\\\\\n";
    print "{\\textit $author}\n";
  }
  else {
